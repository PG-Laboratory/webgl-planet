...
float fresnel(vec3 normal,vec3 vertPos){
    ...
    float otherRefrIndx=1.;// Vaccum or air
    float R0=pow((otherRefrIndx-objRefrInx)/(otherRefrIndx+objRefrInx),2.);
    float cosTheta=dot(normalize(vec3(0.)-vertPos),normal);
...
void main(){
    ...
    if(!useTrochoidalWaves){
        ...
        float displaceAmount=10.*wavesIntensity;
        vec3 gradient=vec3(0.);
        float noise=psrdnoise(vertWorldPos*freqScale,vec3(10.),wavesSpeed*time,gradient);
    ...
    if(useFresnel){
        ...
        float resultAlpha=mix(opacity,1.,reflCoeff);
